...
import { getAppPackageInfo } from '../libs/appinfo';
...
function buildErrorMessage(err, args = [], includeStack = true) {
  ...
  if (err.id) args.push(`  File: ${cyan(err.id)}`);
...
function falconModule(opts = {}) {
  return {
    ...
    buildEnd(error) {
      ...
      if (error) {
        try {
          const msg = buildErrorMessage(error, [red(`构建失败: ${error.message}`)]);
          console.error(msg);
        } catch (_) {
      ...
      if (optionalPkgs.length != 0) {
        warn(
          `未找到以下模块,可能会导致应用运行时异常(如模块为原生模块请忽略):\n${[...optionalPkgs].join(',')}\n`,
        );
