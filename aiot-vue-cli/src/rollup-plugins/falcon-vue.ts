...
import { pathToRootUnix, getAppMeta, getAppRoot } from '../libs/appinfo';

import vueParse from '../libs/parser';
import genId from '../libs/gen-id';
import { readFileSync } from 'fs';

const PACKAGE_NAME = 'aiot-vue-cli';

async function _transform(parts, id) {
  let scopeId = 'data-v-' + genId(relative(process.cwd(), id));
...
${template}
const __file = '${pathToRootUnix(id)}';
...
_exports.style = ${styleInfo.styleString
      ? styleInfo.styleString
      : styleInfo.themesString
        ? "_exports.themes['_']"
        : '{}'
    };
...
function _createStyleString(styles, fileName, scopeId) {
  ...
  styles.forEach((style, index) => {
    importString += `import style_${index} from '${basename(fileName)}?scopeId=${scopeId}&${PACKAGE_NAME}=styles.${index}.${style.lang ? style.lang : 'css'}'\n`;
...
function _createScriptString(script, fileName, scopeId, isFunctional) {
  ...
  return `import script from '${basename(fileName)}?scopeId=${scopeId}&${PACKAGE_NAME}=script.js'`;
}
...
function _template(template, id, isFunctional) {
  ...
  function toFunction(code) {
    return (
      ...
      (isFunctional ? '_h,_vm' : '') +
      '){\n' +
      beautify.js_beautify(code, { indent_size: 2 }) +
      '}'
    );
  }
  let code = `
...
function parseVueQueryString(id) {
  ...
  if (PACKAGE_NAME in query) {
    const data = Array.isArray(query[PACKAGE_NAME]) ? query[PACKAGE_NAME][0] : query[PACKAGE_NAME];
...
function VueTemplate() {
  ...
  return {
    ...
    resolveId(id, importer) {
      ...
      let newId = resolve(dirname(importer), id);
      ...
      if (parts) {
        ...
        if (src) {
          // console.log('src:', src);
          if (src.startsWith('.')) {
            newId = resolve(dirname(importer), src);
          } else if (src.startsWith('@')) {
            newId = resolve(
              dirname(importer),
              src.replace('@', resolve(getAppRoot(), 'src')),
            );
          } else {
    ...
    load(id) {
      ...
      if (meta.type == 'styles') {
        ...
        if (style.src) {
          let filePath = resolve(dirname(id), style.src);
        ...
        if (parts.script) {
          if (parts.script.src) {
            let filePath = resolve(dirname(id), parts.script.src);
