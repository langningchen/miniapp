...
class FalconEvent {
  ...
  unRegister(name, callback) {
    ...
    if (evtList) {
      if (callback) {
        const index = evtList.findIndex(
          (item) => item.callback === callback || item.id === callback,
        );
        if (index !== -1) {
 ...
 */
const COPYS_FROM_APPCONTEXT_TO_FALCON = ['__NAVIGATOR', '__JSAPI'];
...
class Falcon extends FalconEvent {
  ...
  __loadPage(PageClass, pageName, wxInstance, instanceId, Vue, options) {
    ...
    Vue.config.errorHandler = (err, vm, info) => {
      this._vueErrorHandler(err, vm, info);
    };
