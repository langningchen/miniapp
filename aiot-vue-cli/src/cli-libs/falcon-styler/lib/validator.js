...
var COLOR_VALIDATOR = function COLOR_VALIDATOR(v) {
  ...
  return {
    ...
    reason: function reason(k, v, result) {
      return (
        'ERROR: property value `' + v + '` is not valid for `' + util.camelCaseToHyphened(k) + '`'
      );
    },
  };
};
    ...
    return {
      ...
      reason: function reason(k, v, result) {
        return (
          ...
          '` is not supported for `' +
          util.camelCaseToHyphened(k) +
          '` (supported like: `box-shadow:10px 10px 10px #000000`)'
        );
      },
    };
  }
    ...
    return {
      ...
      reason: function reason(k, v, result) {
        return (
          ...
          '` is not supported for `' +
          util.camelCaseToHyphened(k) +
          '` (supported like: `background-position:left top;support values are:`left`,`top`, `right`,`bottom`,`center`,`[number]%`)'
        );
      },
    };
  }
...
var PROP_NAME_GROUPS = {
  ...
  flexbox: {
    ...
    justifyContent: genEnumValidator([
      'flex-start',
      'flex-end',
      'center',
      'space-between',
...
function validate(name, value) {
  ...
  if (typeof validator === 'function') {
    ...
    var suggested = suggestedName
      ? ', suggest `' + util.camelCaseToHyphened(suggestedName) + '`'
      : '';
    log = {
      reason:
        'WARNING: `' +
        util.camelCaseToHyphened(name) +
